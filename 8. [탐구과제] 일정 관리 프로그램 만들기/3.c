...
void renderSchedules (), refresh (), removeLine ();
...
void renderSchedules () {
  ...
  // 시간구하기 {
  time_t todayRaw = time(NULL);
  struct tm today = *localtime(&todayRaw);
  // }
  ...
  switch (displayMod) {
    case 0: ;
      ...
      for (int c = 0; c < GROUPS; c++) {
        ...
        for (int d = 0; d < group[c].length; d++) {
          ...
          /*
            일정의 중요도의 따라 색깔을 다르게 출력하도록 했다
            #1 - 빨강
            #2 - 노랑
            #3 - 초록
            #4 - 파랑

            또한 일정의 날짜가 오늘일 경우 배경색으로 반전 시키도록 했다
          */

          switch (group[c].schedules[d].important) {
            case 1: // 일정 중요도가 #1 일 경우
              if (group[c].schedules[d].date.date == today.tm_mday && group[c].schedules[d].date.month == today.tm_mon + 1&& group[c].schedules[d].date.year == today.tm_year + 1900) printf("\e[7;31m"); // 오늘의 일정일 경우 배경을 빨간색으로 한다
              else printf("\e[0;31m"); // 아닐경우 글씨를 빨간색으로 출력
              break;

            case 2: // 일정 중요도가 #2 일 경우
              if (group[c].schedules[d].date.date == today.tm_mday && group[c].schedules[d].date.month == today.tm_mon + 1&& group[c].schedules[d].date.year == today.tm_year + 1900) printf("\e[7;33m"); // 오늘의 일정일 경우 배경을 노란색으로 한다
              else printf("\e[0;33m"); // 아닐경우 글씨 노란색으로 출력
              break;

            // 초록
            case 3:
              if (group[c].schedules[d].date.date == today.tm_mday && group[c].schedules[d].date.month == today.tm_mon + 1&& group[c].schedules[d].date.year == today.tm_year + 1900) printf("\e[7;32m");
              else printf("\e[0;32m");
              break;

            // 파랑
            case 4:
              if (group[c].schedules[d].date.date == today.tm_mday && group[c].schedules[d].date.month == today.tm_mon && group[c].schedules[d].date.year == today.tm_year + 1900) printf("\e[7;34m");
              else printf("\e[0;34m");
              break;

            // 흰색
            default:
              if (group[c].schedules[d].date.date == today.tm_mday && group[c].schedules[d].date.month == today.tm_mon && group[c].schedules[d].date.year == today.tm_year + 1900) printf("\e[7;0m");
              else printf("\e[0;0m");
              break;
          }
      ...
      printf("\n\e[0;0m중요도: \e[0;31m#1 \e[0;33m#2 \e[0;32m#3 \e[0;34m#4 \e[0;0m#5~ | 오늘의 일정: \e[7;33m배경색 반전\e[0;0m");
      printf("\n총 %d개) [일정 선택: 위, 아래 | 자세히: Space | 일정 추가: i | 일정 수정: e | 일정 삭제: d]", totalSchedules + 1);
      break;
    ...
    case 3: ;
      ...
      if (thisgroup < 0) {
        ...
        char temp[500];
        sprintf(temp, "\n%s %s %s %d %s %s", dateStr, timeStr, importantStr, thisgroup, title, description);
        fputs(temp, file);
      ...
      case 4: // 화면 번호가 4번, 즉 삭제 화면인경우
        removeLine(); // 선택된 일정 삭제
        displayMod = 0; // 화면 번호 초기화
        curserSelect = 0; // 커서 초기화
